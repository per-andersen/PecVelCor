...
# - 
def get_corrected_redshifts_uncertainties(root_dir,mode='diagonal/matrix'):
	'''
	This function takes as input type Ia data and data covariance matrix for the beta uncertainty.
	It returns the data covariance matrix (or just the diagonal thereof) which now includes an
	uncertainty contribution from peculiar velocity correction.
	'''
	
	data = Table.read(root_dir + 'data/DES+nearby/DES+nearby_20171028.fits')
	covar = fits.open(root_dir + 'data/DES+nearby/covmat/C_eta_20171030.fits')
	...
	sigma_redshift = np.sqrt(sigma_lineartheory**2 + sigma_shotnoise**2 + sigma_missingdata**2) / 2.99792e5
	sigma_mu = sigma_redshift * 5./np.log(10.)*((1.+zcor)/zcor/(1.+0.5*zcor))

	
	#plt.figure()
	#plt.plot(sigma_mu,zcor,'x')
	#plt.show()

	print covar[0].data[0,0]
	covar[0].data[np.diag_indices_from(covar[0].data)] = 1.
	print covar[0].data[0,0]

	print np.shape(np.diag(covar[0].data))
	print np.shape(covar[0].data[np.diag_indices_from(covar[0].data)])
	print np.shape(zcor)
	covar.close()
...
def get_sigma_mu_pecvel(redshifts):
	...
	sigma_mu = sigma_redshift * 5./np.log(10.)*((1.+redshifts)/redshifts/(1.+0.5*redshifts))
